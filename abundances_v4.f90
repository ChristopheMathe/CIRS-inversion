...
do 500 iter = 0, niter
    ...
    print*,nb_obs
!~     stop
    call transfer_abundance(nlev,nlay,vmr_tropo_n2,vmr_tropo_h2,vmr_tropo_ar,pl,tl,ml,gl,qch4,ql,p,ap,t,nb_obs,&
    ...
    if(iprint > 1) then
        do 300 ix=1,nx
            ...
            print 350, freq_x(ix)
            350 format(/,' K-matrices at wavenumber =',f8.2,' cm-1:')
            do 301 is=1,nb_obs
                print 351, is
                351 format(5x,'Secant #',i2)
                do 312 ik=1,nb_mol_inv
                    print 352, name_mol_inv(ik)
                    352 format(10x,a5)
                    if(icorps_k(ik) > 0) then
                        print 353, (pl(j),tl(j),ql(j,icorps_k(ik)),matrix_k(ik,j,is,i),j=nlay,1,-1)
                    else
                        print 353, (pl(j),tl(j),taucl_tot(j),matrix_k(ik,j,is,i),j=nlay,1,-1)
                    endif
                    353 format(12x,e14.4,f9.2,e15.4,5x,e15.4)
                312 end do
            301 end do
        300 end do
    end if
...
write(10,1102) (name_mol_inv(ik),ik=1,nb_mol_inv)
1102 format(28x,5(4x,a4,4x,48x))
do 411 j=nlay,1,-1
    write(10,1101) 0.5*(z(j)+z(j+1)),pl(j),tl(j),(ql(j,icorps_k(ik)),&
    errq_inf(j,ik),errq_sup(j,ik),errqb_inf(j,ik),errqb_sup(j,ik),ik=1,nb_mol_inv)
    1101 format(f8.2,e11.4,f9.2,5(5e12.4))
411 END DO
close(10)
!----------------------------------------------------------------------!
