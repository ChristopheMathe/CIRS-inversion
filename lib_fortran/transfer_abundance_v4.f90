...
do j = nlay, 1, -1
    ...
    if(icloud /= 0) then
        do ik = 1, n_sort
            f_cent = f1 + pas_sort * (dfloat(ik) - 5d-1)
            cloudj = 0d0
            do ic = 1, icloud
...
!            call get_line_absorption_cross_sections(w_out(k,:), s_out(k,:), e_out(k,:), n_out(k,:), nlines(k), vgt, &
!                                                    nvoigt, nvoigt, nbl_sp_max, nstep, nstep0, stepj, f1, f2, hckt, &
!                                                    hck296, fdop, dtauk)
...
deallocate(etau00, etau, etau0, etau0_k, etau_k, etau_k2, detau0, dtauk0, dtauk, dtau_k0, dtau_k, pl_lay, pl_ground, &
